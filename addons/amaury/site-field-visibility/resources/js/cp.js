 ...
 * Automatically hide/show fields based on the active site in the control panel
 */
...
Statamic.booting(() => {
    ...
    const hideByHandle = (handle) => {
        ...
        console.log('[SiteFieldVisibility] HideField', handle, 'total hidden:', totalHidden);
        return totalHidden;
    };
    ...
    const showByHandle = (handle) => {
        ...
        // eslint-disable-next-line no-console
        console.log('[SiteFieldVisibility] Looking for field:', handle);
        
        let totalShown = 0;
        selectors.forEach(selector => {
            const nodes = document.querySelectorAll(selector);
            // eslint-disable-next-line no-console
            console.log('[SiteFieldVisibility] Selector:', selector, 'found', nodes.length, 'nodes');
            nodes.forEach(n => { 
                if (n) {
                    // eslint-disable-next-line no-console
                    console.log('[SiteFieldVisibility] Before - display:', n.style.display, 'visibility:', n.style.visibility, 'classList:', n.classList.toString());
                    ...
                    while (parent && level < 5) {
                        if (parent.style.display === 'none') {
                            ...
                            // eslint-disable-next-line no-console
                            console.log('[SiteFieldVisibility] Fixed parent display:', parent);
                        }
                    ...
                    // eslint-disable-next-line no-console
                    console.log('[SiteFieldVisibility] After - display:', n.style.display, 'visibility:', n.style.visibility, 'classList:', n.classList.toString());
                    totalShown++;
                }
            });
        });
        ...
        // eslint-disable-next-line no-console
        console.log('[SiteFieldVisibility] ShowField', handle, 'total shown:', totalShown);
        return totalShown;
    };

    const detectActiveSite = (store) => {
        // eslint-disable-next-line no-console
        console.log('[SiteFieldVisibility] store.state:', store?.state);
        // eslint-disable-next-line no-console
        console.log('[SiteFieldVisibility] store.getters:', store?.getters);

        if (store?.state?.site) {
            console.log('[SiteFieldVisibility] via state.site ->', store.state.site);
            return store.state.site;
        }
        if (store?.getters?.site) {
            console.log('[SiteFieldVisibility] via getters.site ->', store.getters.site);
            return store.getters.site;
        }
        if (store?.state?.values?.site) {
            console.log('[SiteFieldVisibility] via values.site ->', store.state.values.site);
        ...
        const cfg = (window.Statamic && window.Statamic.config && window.Statamic.config.site && window.Statamic.config.site.handle)
            ...
            : 'default';
        console.log('[SiteFieldVisibility] fallback config ->', cfg);
        return cfg;
    };
    ...
    const applyPerSiteHiding = (activeSite, retryCount = 0) => {
        ...
        if (hiddenCount === 0 && retryCount < 3 && document.querySelector('.publish-form')) {
            console.log('[SiteFieldVisibility] No fields hidden, retrying in 500ms (attempt', retryCount + 1, ')');
    ...
    const onPublishBooted = ({ store }) => {
        ...
        if (!activeSite || activeSite === 'default') {
            if (window.__ACTIVE_SITE && typeof window.__ACTIVE_SITE === 'string') {
                console.log('[SiteFieldVisibility] using observed __ACTIVE_SITE =', window.__ACTIVE_SITE);
        ...
        if (!activeSite || activeSite === 'default') {
            ...
            if (formSiteEl && formSiteEl.value) {
                ...
                console.log('[SiteFieldVisibility] found hidden form site =', activeSite);
            }
        }
        console.log('[SiteFieldVisibility] publish:booted activeSite =', activeSite);
    ...
    const updateObservedSite = (site) => {
        ...
        console.log('[SiteFieldVisibility] ObservedSite set to', site);
    ...
    } catch (e) { console.log('[SiteFieldVisibility] HookFetch error', e); }
